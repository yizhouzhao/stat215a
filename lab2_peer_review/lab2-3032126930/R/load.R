...
loadMoteLocationData <- function(path = "./data/") {
  ...
  # load in the txt table
  mote_location <- read.table(paste0(path, "mote-location-data.txt"), 
                              header = T)
...
loadLingData <- function(path = "./data/") {
  # Arguments:
  #   path: the path indicating the location of the `lingData.txt` data files.
  #         Path should be relative to the lab2.Rnw file.
  
  # load in the txt table
  ling_data <- read.table(paste0(path, "lingData.txt"), 
                              header = T)
  return(ling_data)
}
...
loadLingLocation <- function(path = "./data/") {
  # Arguments:
  #   path: the path indicating the location of the `lingLocation.txt` data files.
  #         Path should be relative to the lab2.Rnw file.
  
  # load in the txt table
  ling_location <- read.table(paste0(path, "lingLocation.txt"), 
                          header = T)
